...
class Solver3:
        ...
        for i in range(0, len(self.graph.nodes)):
            for j in range(0, len(self.graph.nodes)):
                ...
                for k in range(0, len(self.graph.nodes)):
                    ...
                    self.generate_next_triangle(solution_triangles.copy(), used_nodes.copy(), free_edges.copy())
    ...
    def generate_next_triangle(self, solution_triangles, used_nodes, free_edges):
        for i in free_edges:
            for j in self.graph.nodes:
                if j in used_nodes:
                    continue
                ...
                if self.number_of_solutions > 4000000:
                    return
                ...
                if set(self.graph.nodes).issubset(set(used_nodes_copy)):
                    # print("Solution: ")
                    # for t in solution_triangles_copy:
                    #     for edge in t:
                    #         print(edge)
                    #     print("|")
                    solution_cost = self.calculate_solution_value(solution_triangles_copy)
                    if len(self.result_triangles) == 0:
                        self.min_cost = solution_cost
                    
                    if solution_cost == self.min_cost and len(self.result_triangles) < self.max_number_of_solutions:
                        self.result_triangles.append(solution_triangles_copy)
                    elif solution_cost < self.min_cost:
                        self.result_triangles = []
                        self.result_triangles.append(solution_triangles_copy)
                        self.min_cost = solution_cost
                    if solution_cost == 728:
                        print(str(solution_cost))
                        # for t in solution_triangles_copy:
                        #     for edge in t:
                        #         print(edge)
                        #     print("|")
                    ...
                    self.generate_next_triangle(solution_triangles_copy, used_nodes_copy, free_edges_copy)
